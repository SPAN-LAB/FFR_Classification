...
def generic_analyze(
    ...
    MIN_SUBAVERAGE_SIZE = 5
    SUBAVERAGE_STRIDE = 5
    MAX_SUBAVERAGE_SIZE = 120
    INCLUDE_NULL_CASE = True
    SUBAVERAGE_SIZES = list(range(MIN_SUBAVERAGE_SIZE, MAX_SUBAVERAGE_SIZE, SUBAVERAGE_STRIDE))
    if INCLUDE_NULL_CASE:
    ...
    for subaverage_size in SUBAVERAGE_SIZES:
        ...
        train_pipeline.subaverage(subaverage_size)
        pre_test_pipeline.subaverage(subaverage_size)
        pre_test_pipeline.fold(defaults.NUM_FOLDS)
        post_test_pipeline.subaverage(subaverage_size)
        post_test_pipeline.fold(defaults.NUM_FOLDS)
        test_pipeline.subaverage(subaverage_size)
