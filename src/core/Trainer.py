...
from .eeg_subject import EEGSubjectInterface, EEGSubject
...
class TrainerInterface(ABC):
    ...
    @abstractmethod
    def train(
        self, use_gpu: bool, num_epochs: int, lr: float, stopping_criteria: bool
    ) -> None: ...
...
class Trainer(TrainerInterface):
    ...
    def _trials_to_np(
        self, trials: list[EEGTrial], *, add_channel_dim: bool, adjust_labels: bool
    ):
        X = np.stack([t.data for t in trials], axis=0).astype(np.float32)
        y = np.asarray([int(t.raw_label) for t in trials], dtype=np.int64)
    ...
    def create_train_val_dls(
        ...
        *,
        ...
        sss = StratifiedShuffleSplit(
            n_splits=1, test_size=val_frac, random_state=42 + test_fold_idx
        )
        tr_idx, val_idx = next(sss.split(np.arange(n), y_tr_full))
    ...
    def create_test_dl(
        ...
        *,
        ...
        ds = TensorDataset(
            torch.from_numpy(X_te), torch.from_numpy(y_te), torch.from_numpy(idx_te)
        )
        return DataLoader(ds, batch_size=batch_size, shuffle=False)
    ...
    def train(self, use_gpu: bool, num_epochs: int, lr: float, stopping_criteria: bool):
        ...
        for fold_idx in range(len(folds)):
            ...
            for epoch in range(1, num_epochs + 1):
                ...
                with torch.no_grad():
                    ...
                    if val_acc > best_val_acc + min_impr:
                        ...
                        torch.save(
                            model.state_dict(),
                            (fold_dir / "checkpoints" / "best.pt").as_posix(),
                        )
                    else:
                        if stopping_criteria:
                            no_improve += 1
            ...
            torch.save(
                model.state_dict(), (fold_dir / "checkpoints" / "last.pt").as_posix()
            )
            with open((fold_dir / "metrics.json").as_posix(), "w") as f:
                json.dump(
        ...
        print(
            f"[{self.subject_name}] {self.model_name} | folds={len(folds)} | mean_best_val_acc={mean_best:.3f}"
        )
    ...
    def test(self):
        ...
        for fold_idx in range(num_folds):
            ...
            test_dl = self.create_test_dl(
                test_fold_idx=fold_idx, add_channel_dim=add_channel_dim
            )
            ...
            if not weights_path.exists():
                raise FileNotFoundError(
                    f"No checkpoint for fold {fold_idx + 1} in {ckpt_dir}"
                )
            ...
            with open(fold_dir / "test_results.json", "w") as f:
                json.dump(
                    {"test_acc": float(acc), "confusion_matrix": cm.tolist()},
                    f,
                    indent=2,
                )
            ...
            print(
                f"[{self.subject_name}] test fold {fold_idx + 1}: acc = {acc:.3f} n = {y_true.size}"
            )
        ...
        mean_acc = total_acc / num_folds if num_folds > 0 else 0.0
        overall_acc = float((oof_pred == oof_true).mean())  # <-- NEW

        print(f"[{self.subject_name}] overall accuracy = {overall_acc:.3f}")  # <-- NEW
        ...
        with open(subject_dir / "overall_metrics.json", "w") as f:
            json.dump(
                {
                    "mean_acc": float(mean_acc),
                    "overall_acc": float(overall_acc),  # <-- NEW
                    "confusion_matrix_overall": cm_overall.tolist(),
                },
                f,
                indent=2,
        ...
        with open(subject_dir / "roc_overall.json", "w") as f:
            json.dump(
                {"per_class_auc": aucs, "micro_auc": float(auc_micro)}, f, indent=2
            )
