...
class _AutocorrCNN1D(nn.Module):
    """
    Same architecture as CNN but operating on the full-signal autocorrelation
    (~4997 samples, same length as raw input).
...
class AutocorrCNNModel(TorchNNBase):
    required_inputs = ["autocorr"]
    ...
    def build(self) -> None:
        ...
        self.model = _AutocorrCNN1D(n_classes=n_classes, p_drop=p_drop).to(self.device)
